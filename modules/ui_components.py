...
from constants import (
    DEFAULT_START_NODE, DEFAULT_TARGET_NODE, DEFAULT_LAYOUT_SEED,
    MIN_NODES, MAX_NODES, DEFAULT_NODES,
    MIN_NEIGHBORS, MAX_NEIGHBORS, DEFAULT_NEIGHBORS,
    MIN_REWIRE_PROB, MAX_REWIRE_PROB, DEFAULT_REWIRE_PROB,
    COLOR_ACTIVE, COLOR_INACTIVE, COLOR_START_NODE, COLOR_TARGET_NODE,
    STEP_HEADINGS, ERROR_NODE_NOT_IN_GRAPH, ERROR_INVALID_DATA
)
    ...
    return ui.page_fluid(
        ui.layout_sidebar(
            ui.sidebar(
                ...
                ui.input_numeric("start_node", ui.span("Start Node", ui.output_ui("start_node_error_message")), 
                               value=DEFAULT_START_NODE, min=0),
                ui.input_numeric("target_node", ui.span("Target Node", ui.output_ui("target_node_error_message")),
                               value=DEFAULT_TARGET_NODE, min=0),
                ui.input_numeric("layout_seed", "Layout Seed", value=DEFAULT_LAYOUT_SEED, min=0),
            ),
...
def render_graph_generator_settings(graph_type):
    ...
    if graph_type == GraphType.RANDOM_GRAPH.value:
        return ui.TagList(
            ui.input_slider("n_slider", "Number of Nodes", MIN_NODES, MAX_NODES, DEFAULT_NODES),
            ui.input_slider("k_slider", "Neighbors in a ring topology", MIN_NEIGHBORS, MAX_NEIGHBORS, DEFAULT_NEIGHBORS),
            ui.input_slider("p_slider", "Probability of rewiring each edge", MIN_REWIRE_PROB, MAX_REWIRE_PROB, DEFAULT_REWIRE_PROB),
        )
...
def render_distances_table(df, start_node, target_node):
    ...
    if contains(df.columns, "Node"):
        try:
            index_start = int(df.index[df['Node'].astype(int) == int(start_node)].item())
            index_target = int(df.index[df['Node'].astype(int) == int(target_node)].item())
        except ValueError:
        ...
        try:
            styles = [
                ...
                {"rows": index_target, "style": {"background-color": COLOR_TARGET_NODE}},
            ]
        ...
        try:
            styles = [
                {"rows": [int(start_node)], "style": {"background-color": COLOR_START_NODE}},
                {"rows": [int(target_node)], "style": {"background-color": COLOR_TARGET_NODE}},
            ]
            return render.DataTable(df, width="100%", styles=styles)
        except TypeError:
            df = pd.DataFrame({"Error": [ERROR_INVALID_DATA]})
